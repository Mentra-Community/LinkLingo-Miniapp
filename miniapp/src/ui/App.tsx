...
/**
 * Free-form comment box. Whatever the user types goes to the backend with the
 * last few translations, the recent speech and the tape, and a smarter model
 * answers in plain text. Exchanges are archived (`bun run review -- --feedback`).
 */
  ...
  useEffect(() => {
    return mentra.on("link:feedback-result", (payload) => {
      setPending((current) => {
        ...
        setResult(payload.ok ? {analysis: payload.analysis} : {error: payload.error ?? "No answer"})
        return null
      })
    })
  }, [])
  ...
  return (
    <div className="analyst">
      <textarea
        ...
        rows={2}
        placeholder="Comment on the last translations…"
        value={note}
        disabled={pending !== null}
        onChange={(e) => setNote(e.target.value)}
      />
      <button type="button" className="primary" disabled={pending !== null || note.trim().length === 0} onClick={submit}>
        {pending ? `Thinking… ${elapsed}s` : "Send"}
      </button>
      {result?.error ? (
      ...
      {result?.analysis ? (
        <div className="analysis">
          <p className="analysis-text">{result.analysis.answer}</p>
          <p className="meta">
            {result.analysis.model} · {(result.analysis.totalMs / 1000).toFixed(1)}s · saved
          </p>
        </div>
      ) : null}
    </div>
